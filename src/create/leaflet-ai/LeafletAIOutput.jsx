...
function LeafletAIOutput({ userSection, setUserSection, suggestedName, onBack, onImported }) {
  ...
  const fullPrompt = `${LEAFLET_PROMPT_BASE}\n\n${userSection}`
  ...
  const handleGenerate = async () => {
    ...
    try {
      const result = await window.sensei.anthropic.generate({
        ...
        systemPrompt: LEAFLET_PROMPT_BASE,
      })
      if (result?.success) {
  ...
  const handleImport = async () => {
    setImporting(true)
    setImportError(null)
    const slug     = (artifactName || suggestedName || 'leaflet').toLowerCase().replace(/\s+/g, '-').replace(/[^a-z0-9-]/g, '')
        ...
        {genState === 'done' ? (
          <>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
              ...
              <span style={{ fontSize: 14, fontWeight: 600, color: 'var(--text-primary)' }}>Leaflet generato</span>
            </div>
          </>
        ) : (
          <>
            ...
            <div>
              ...
              <span style={{ fontSize: 12, color: 'var(--text-tertiary)', marginLeft: 10 }}>Personalizza e genera il leaflet</span>
            </div>
          </>
        )}
      </div>
          ...
          {genState === 'done' ? (
            ...
            <>
              <div style={{ background: 'var(--bg-secondary)', border: '0.5px solid var(--border)', borderRadius: 'var(--radius-lg)', overflow: 'hidden' }}>
                <div style={{ padding: '10px 16px', borderBottom: '0.5px solid var(--border)', display: 'flex', alignItems: 'center', gap: 8 }}>
                  ...
                  <span style={{ fontSize: 11, color: 'var(--text-tertiary)', fontFamily: 'monospace' }}>leaflet.jsx</span>
                </div>
              ...
              {validationWarnings.length > 0 && (
                <div style={{ marginTop: 12, padding: '12px 14px', borderRadius: 'var(--radius-md)', background: '#BA751712', border: '0.5px solid #BA751744' }}>
                  <p style={{ fontSize: 12, fontWeight: 500, color: '#BA7517', margin: '0 0 6px' }}>
                    Avvisi — il leaflet potrebbe funzionare con limitazioni
                  </p>
            ...
            <>
              ...
              <div style={{ marginBottom: genState === 'error' ? 12 : 0 }}>
                <p style={{ fontSize: 13, fontWeight: 500, color: 'var(--text-primary)', margin: '0 0 3px' }}>Il tuo leaflet</p>
      ...
      {showSaveDialog && (
        ...
        >
          <div style={{ background: 'var(--bg-primary)', border: '0.5px solid var(--border)', borderRadius: 'var(--radius-lg)', padding: 24, width: 420, maxHeight: '85vh', overflowY: 'auto' }}>
            <h2 style={{ fontSize: 15, fontWeight: 600, marginBottom: 4, color: 'var(--text-primary)' }}>Importa leaflet</h2>
            ...
            <input
              ...
              autoFocus
              placeholder="Es. Guida configurazione OBS"
...
export default LeafletAIOutput
