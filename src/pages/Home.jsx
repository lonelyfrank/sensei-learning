...
// Filtri disponibili per la sezione "Tutti i corsi"
...
function Home({ courses, onSelectCourse, onImport, onRemove }) {
  ...
  // Dividi i corsi nelle tre categorie
  const inProgress = courses
    .filter(c => c.progress > 0 && c.progress < 100)
    .sort((a, b) => b.progress - a.progress) // più vicini alla fine prima
    ...
    return (
      ...
      >
        ...
        <ContextMenu
          ...
          onRemove={() => { setMenuOpen(false); onRemove(course) }}
          menuRef={menuRef}
        />
      </div>
    )
    ...
    >
      ...
      <div onClick={onClick}>
        <div style={{
          ...
          {isCompleted ? <CheckIcon color="#1D9E75" /> : <CourseIcon color={course.color} />}
        </div>
        ...
        {!isCompleted && (
          <p style={{ margin: '6px 0 0', fontSize: 11, color: course.color, textAlign: 'right' }}>
            {course.progress}%
          </p>
        )}
      </div>
